...
from models.networks.stylegan2_layers import ResBlock, ConvLayer, EqualLinear, Blur, Upsample, make_kernel, ConditionalEmbedding2d
...
class StyleGAN2ResnetEncoder(BaseNetwork):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument("--netE_num_downsampling_sp", default=2, type=int)
        parser.add_argument("--netE_num_downsampling_gl", default=4, type=int)
    ...
    def __init__(self, opt):
        ...
        for i in range(self.opt.netE_num_downsampling_sp):
            self.DownToSpatialCode.add_module("ResBlockDownBy%d" % (2 ** i), ResBlock(self.nc(i), self.nc(i + 1), blur_kernel, reflection_pad=True))
        ...
        self.add_module("ToSpatialCode", nn.Sequential(ConvLayer(nchannels, nchannels, 1, activate=True, bias=True),
        ...
        for i in range(self.opt.netE_num_downsampling_gl):
            ...
            self.DownToGlobalCode.add_module("ConvLayerDownBy%d" % (2 ** idx_from_beginning), ConvLayer(self.nc(idx_from_beginning), self.nc(idx_from_beginning + 1), kernel_size=3, blur_kernel=[1], downsample=True, pad=0))
