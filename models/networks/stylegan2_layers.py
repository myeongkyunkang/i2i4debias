...
class StyledConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, style_dim, upsample=False, blur_kernel=[1, 3, 3, 1], demodulate=True, use_noise=True, lr_mul=1.0, ):
        super().__init__()
...
class ConvLayer(nn.Sequential):
    def __init__(self, in_channel, out_channel, kernel_size, downsample=False, blur_kernel=[1, 3, 3, 1], bias=True, activate=True, pad=None, reflection_pad=False, ):
        layers = []
        ...
        layers.append(("Conv", EqualConv2d(in_channel, out_channel, kernel_size, padding=self.padding, stride=stride, bias=bias and not activate, )))
...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, blur_kernel=[1, 3, 3, 1], reflection_pad=False, pad=None, downsample=True):
        ...
        self.skip = ConvLayer(in_channel, out_channel, 1, downsample=downsample, blur_kernel=blur_kernel, activate=False, bias=False)
...
class ConditionalEmbedding2d(nn.Module):
    def __init__(self, num_features, num_classes=50):
        super().__init__()
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], direction=False):
        super().__init__()
        ...
        self.final_linear = nn.Sequential(EqualLinear(channels[4] * (side_length ** 2), channels[4], activation='fused_lrelu'),
