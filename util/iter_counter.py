...
class IterationCounter():
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument("--evaluation_freq", default=100000, type=int)
        parser.add_argument("--print_freq", default=480, type=int)
        parser.add_argument("--display_freq", default=5000, type=int)
        return parser
    ...
    def __init__(self, opt):
        ...
        automatically_find_resume_iter = opt.isTrain and opt.continue_train and opt.resume_iter == "latest" and opt.pretrained_name is None
        resume_at_specified_iter = opt.isTrain and opt.continue_train and opt.resume_iter.replace("k", "").isnumeric()
        if automatically_find_resume_iter:
            try:
                self.steps_so_far = np.loadtxt(self.iter_record_path, delimiter=',', dtype=int)
    ...
    def record_one_iteration(self):
        if self.needs_saving():
            np.savetxt(self.iter_record_path, [self.steps_so_far], delimiter=',', fmt='%d')
    ...
    def needs_evaluation(self):
        return (self.steps_so_far >= self.opt.evaluation_freq) and ((self.steps_so_far % self.opt.evaluation_freq) < self.batch_size)
