...
class OursOptimizer(BaseOptimizer):
    ...
    def __init__(self, model: MultiGPUModelWrapper):
        ...
        self.optimizer_G = torch.optim.Adam(self.Gparams, lr=opt.lr, betas=(opt.beta1, opt.beta2))

        # c.f. StyleGAN2 (https://arxiv.org/abs/1912.04958) Appendix B
        c = opt.R1_once_every / (1 + opt.R1_once_every)
        self.optimizer_D = torch.optim.Adam(self.Dparams, lr=opt.lr * c, betas=(opt.beta1 ** c, opt.beta2 ** c))
    ...
    def prepare_images(self, data_i):
        ...
        images_A, images_B = data_i['real_B' if BtoA else 'real_A'], data_i['real_A' if BtoA else 'real_B']
        labels_A, labels_B = None, None
        if 'label_A' in data_i and 'label_B' in data_i:
            labels_A, labels_B = data_i['label_B' if BtoA else 'label_A'], data_i['label_A' if BtoA else 'label_B']
        return images_A, images_B, labels_A, labels_B
    ...
    def train_one_step(self, data_i, total_steps_so_far):
        images_minibatch_A, images_minibatch_B, labels_minibatch_A, labels_minibatch_B = self.prepare_images(data_i)
        if self.toggle_training_mode() == "generator":
            losses = self.train_discriminator_one_step(images_minibatch_A, images_minibatch_B, labels_minibatch_A, labels_minibatch_B)
        else:
            losses = self.train_generator_one_step(images_minibatch_A, images_minibatch_B, labels_minibatch_A, labels_minibatch_B)
        return util.to_numpy(losses)

    def train_generator_one_step(self, images_A, image_B, labels_A, labels_B):
        self.set_requires_grad(self.Dparams, False)
        self.set_requires_grad(self.Gparams, True)
        self.optimizer_G.zero_grad()
        g_losses, g_metrics = self.model(images_A, image_B, labels_A, labels_B, command="compute_generator_losses")
    ...
    def train_discriminator_one_step(self, images_A, image_B, labels_A, labels_B):
        ...
        d_losses, d_metrics = self.model(images_A, image_B, labels_A, labels_B, command="compute_discriminator_losses")
        ...
        if needs_R1_at_current_iter:
            self.optimizer_D.zero_grad()
            r1_losses = self.model(image_B, labels_B, command="compute_R1_loss")
    ...
    def get_visuals_for_snapshot(self, data_i):
        images_A, images_B, labels_A, labels_B = self.prepare_images(data_i)
        with torch.no_grad():
            return self.model(images_A, images_B, labels_A, labels_B, command="get_visuals_for_snapshot")
